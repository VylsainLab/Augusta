...
namespace aug
{
	...
	void aug::Pipeline::RegisterResource(DescriptorSetLayoutHandle h, uint8_t uiBinding, DescriptorTarget* pResource)
	{
	...
	void aug::Pipeline::BindResource(const VkCommandBuffer& cb, DescriptorSetLayoutHandle h, DescriptorTarget* pResource)
	{
		VkDescriptorSet s = DescriptorFactory::GetDescriptorSet(pResource->GetDescriptorSetHandle(h));
		vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, m_VkPipelineLayout, 0, 1, &s, 0, nullptr);
	}
