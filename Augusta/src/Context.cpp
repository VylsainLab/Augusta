...
namespace aug
{
	...
	void Context::PickPhysicalDevice() //select first found suitable device
	{
		...
		vkEnumeratePhysicalDevices(aug::Context::m_VkInstance, &uiDeviceCount, nullptr);
		if (uiDeviceCount == 0)
			throw std::runtime_error("Failed to find GPUs with Vulkan support!");

		std::vector<VkPhysicalDevice> vDevices(uiDeviceCount);
		vkEnumeratePhysicalDevices(aug::Context::m_VkInstance, &uiDeviceCount, vDevices.data());
	...
	void Context::CreateLogicalDevice()
	{
		...
		if (ENABLE_VALIDATION_LAYERS)
		{
			createInfo.enabledLayerCount = static_cast<uint32_t>(aug::Context::m_vValidationLayers.size());
			createInfo.ppEnabledLayerNames = aug::Context::m_vValidationLayers.data();
		}
		else
			createInfo.enabledLayerCount = 0;
