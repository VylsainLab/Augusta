...
namespace vkw
{
	...
	SwapChain::SwapChain(const VkSurfaceKHR& surface)
	{
		...
		vkw::QueueFamilyIndices indices = vkw::Context::GetQueueFamilies(surface);
		...
		vkGetSwapchainImagesKHR(vkw::Context::m_VkDevice, m_VkSwapChain, &m_uiImageCount, nullptr);
		m_vVkSwapChainImages.resize(m_uiImageCount);
		vkGetSwapchainImagesKHR(vkw::Context::m_VkDevice, m_VkSwapChain, &m_uiImageCount, m_vVkSwapChainImages.data());
		...
		for (size_t i = 0; i < m_vVkSwapChainImages.size(); ++i)
		{
			...
			if (vkCreateImageView(vkw::Context::m_VkDevice, &ivcreateInfo, nullptr, &m_vVkSwapChainImageViews[i]) != VK_SUCCESS)
